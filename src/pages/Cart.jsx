        ...
        <div className="bg-gray-50 min-h-screen pt-10 pb-20">
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-8">
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
                    {/* Left: Cart Items */}
                    <div className="lg:col-span-2 flex-1 space-y-6 md:space-y-8">
                        <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm">
                            <div className="p-6 md:p-8 border-b border-gray-50 flex items-center justify-between flex-wrap gap-4">
                                <h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Your Archive <span className="text-amber-500">Selection</span></h1>
                                <span className="bg-gray-100 px-4 py-1.5 rounded-full text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">{cart.length} Items</span>
                            </div>
                                        ...
                                        <div className="flex-1 w-full space-y-3 md:space-y-2">
                                            <div className="flex justify-between items-start gap-4">
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 text-gray-300 hover:text-red-500 transition-colors hover:bg-red-50 rounded-xl flex-shrink-0"
                                                >
                                                    <DeleteOutline />
                                                </button>
                                            </div>

                                            <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-4 pt-2 md:pt-4">
                                                <div className="flex items-center bg-gray-50 rounded-2xl border border-gray-100 p-1">
                                                    ...
                                                    <span className="w-8 md:w-10 text-center font-bold text-gray-900 text-sm md:text-base">{item.quantity}</span>
                                                    <button
                                                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                                                        className="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-amber-600"
                                                    >
                                                ...
                                                <p className="text-xl md:text-2xl font-black text-gray-900">₹{(item.price * item.quantity).toLocaleString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-gray-100 shadow-sm space-y-4">
                            <div className="flex justify-between items-center text-[10px] md:text-sm font-bold flex-wrap gap-2">
                    ...
                    <div className="lg:w-[400px] space-y-6 md:space-y-8">
                        <div className="bg-gray-900 text-white p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl relative overflow-hidden">
                            ...
                            <div className="relative z-10 space-y-6 md:space-y-8">
                                <h2 className="text-xl md:text-2xl font-black tracking-tight border-b border-white/10 pb-4 md:pb-6">Investment <span className="text-amber-400">Summary</span></h2>

                                <div className="space-y-4 border-b border-white/10 pb-8">
                                    <div className="flex justify-between items-center text-[10px] md:text-sm uppercase tracking-widest font-bold">
                                ...
                                <div className="flex justify-between items-center pb-6 md:pb-10">
                                    <span className="text-xs md:text-sm font-black uppercase tracking-widest">Total Amount</span>
                                    <span className="text-2xl md:text-3xl font-black text-amber-400">₹{Math.round(total).toLocaleString()}</span>
                                </div>

                                <button className="group w-full py-4 md:py-5 bg-amber-500 hover:bg-amber-400 text-gray-900 font-black rounded-2xl transition-all duration-300 flex items-center justify-center gap-3 shadow-xl shadow-amber-500/20 active:scale-95 text-sm md:text-base">
                                    Finalise Checkout
                                    <ArrowForward className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
                                </button>

                                <div className="space-y-3 md:space-y-4 pt-4 md:pt-6">
                                    <div className="flex items-center gap-4 text-[8px] md:text-[10px] text-white/40 uppercase tracking-widest font-bold">
                        ...
                        {/* Back Link */}
                        <Link to="/products" className="block text-center text-gray-400 hover:text-gray-900 transition-colors font-bold uppercase tracking-[0.2em] text-[10px] pb-4">
